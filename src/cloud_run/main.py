    ...
    try:
        ...
        try:
            ...
            if not (title and artist):
                logger.error(
                    "[Song Processor] Title or artist is missing in the song info"
                )
            ...
            if not track_metadata:
                logger.warning(
                    f"[Song Processor] No metadata found for '{title}' by '{artist}'"
                )
                return (
                    "Bad Request: No metadata found for the song "
                    "- probably niche song or not a song at all",
                    400,
                )
            # elif track_metadata['track_name'] != title
            # or artist not in track_metadata['artist_names']:
            #     logger.warning(f"[Song Processor] Metadata mismatch
            #     for '{title}' by '{artist}' - Found '{track_metadata['track_name']}'
            #     by '{track_metadata['artist_names']}'")
            #     return "Bad Request: Metadata mismatch", 400

            # Log metadata
            logger.info(
                f"[Song Processor] Found metadata for '{title}' by '{artist}': {track_metadata}"
            )
            ...
            try:
                ...
                if video_info:
                    logger.info(
                        f"Found YouTube video: {video_info['title']} - {video_info['url']}"
                    )
            ...
            logger.info("[Song Processor] Successfully connected to the database")
            ...
            logger.info(
                f"[Song Processor] Successfully loaded song "
                f"'{title}' by '{artist}' to the database with ID {track_id}"
            )
            ...
            logger.info(
                f"[Song Processor] Successfully processed song: {title} by {artist}"
            )
            ...
            logger.error(
                f"[Song Processor] Invalid JSON: {message_data} - Error: {str(e)}"
            )
